...
-(void)initUI
{
    ...
    CGFloat DownY = SCREEN_HEIGHT - 49;
    [DownView setTag:10];
    [DownView setFrame:(CGRect){{0,DownY},{SCREEN_WIDTH, 49}}];
...
-(void)CirculatingItmes
{
    ...
    for (MenuLabel *Obj in _ItmesArr) {
        ...
        if (index == 0)
        {
            buttonX = SCREEN_WIDTH/2 - ButtonWidth - 10;
        }
        else if (index == 1)
        {
            buttonX = SCREEN_WIDTH/2 + 10;
        }
        
        buttonY = SCREEN_HEIGHT - ButtonHigh - 100;
        
//        buttonX = (index % HowMucHline) * ButtonWidth;
//        buttonY = ((index / HowMucHline) * (ButtonHigh +10)) + (SCREEN_HEIGHT/2.85) + 90;
...
//        buttonX = (index % HowMucHline) * ButtonWidth;
//        buttonY = ((index / HowMucHline) * (ButtonHigh +10)) + (SCREEN_HEIGHT/2.9) +90;
...
- (void)selectd:(CustomButton *)button
{
    ...
    //后隐藏
    [self HidDelay:0.2f CompletionBlock:nil];

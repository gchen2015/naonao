...
@implementation STBrowserPhoto

- (void)setPhotoObj:(id)photoObj{
    _photoObj = photoObj;
    
    if ([photoObj isKindOfClass:[STPhotoAssets class]]) {
        STPhotoAssets *asset = (STPhotoAssets *)photoObj;
        self.asset = asset;
    }
...
+ (instancetype)photoAnyImageObjWith:(id)imageObj{
    STBrowserPhoto *photo = [[self alloc] init];
...
- (void)notifyImageDidStartLoad
{
    dispatch_async(dispatch_get_main_queue(), ^{
        _isLoading = YES;
        [[NSNotificationCenter defaultCenter] postNotificationName:STPhotoImageDidStartLoad object:self];
    });
}
...
- (void)notifyImageDidFinishLoad
{
    dispatch_async(dispatch_get_main_queue(), ^{
        _isLoading = NO;
        [[NSNotificationCenter defaultCenter] postNotificationName:STPhotoImageDidFinishLoad object:self];
    });
}
...
- (void)notifyImageDidFailLoadWithError:(NSError *)error
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:STPhotoImageDidFailLoadWithError object:self userInfo:notifyInfo];
    });
}

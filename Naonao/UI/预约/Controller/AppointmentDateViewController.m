...
@interface AppointmentDateViewController ()<SegmentTapViewDelegate, AppointmentViewDelegate>
...
- (void)drawDatesUI{
    ...
    //地址
    UIImageView *pointV = [[UIImageView alloc] initWithFrame:CGRectMake(25, mH+45, 14, 18)];
    [pointV setImage:[UIImage imageNamed:@"cell_address_icon.png"]];
    [mV addSubview:pointV];
    
    //添加点击事件
    UIButton *addressBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, mH+34, SCREEN_WIDTH, 40)];
    ...
    //分割线
    UILabel *lineA = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(pointV.frame)+15, mH+47, 0.8, 14)];
    [lineA setBackgroundColor:LINE_COLOR];
    [mV addSubview:lineA];
    
    UILabel *addressL = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(pointV.frame)+30, mH+45, SCREEN_WIDTH - 53, 18)];
...
- (void)calendarButtonTapped:(UIButton *)sender{
    ...
    NSString *st = [NSString stringWithFormat:@"%@  \n%@--%@的试衣服务", times, self.bookData.address.address, self.bookData.address.name];
    _reserveTime = [NSString stringWithFormat:@"%@ %@", _dateS, calender.time];
    
    //@"当面取货，还可获得专业化妆、发型，专业摄影棚拍摄写真服务~"
    self.actionSheet = [[AppointmentView alloc] initWithTitle:@"是否预约？" delegate:self message:st instructions:nil leftButtonTitle:@"放弃" rightButtonTitle:@"预约"];
...
- (void)didClickOnSureButton:(BOOL)isTure {
    ...
    if (isTure) {
        ...
        [dic setObject:_skuID forKey:@"sku_id"];
        
        __typeof (&*self) __weak weakSelf = self;
        [[AppointmentLogic sharedInstance] getAppointmentTry:dic withCallback:^(LogicResult *result) {
            if(result.statusCode == KLogicStatusSuccess)
            {
                [weakSelf showAlert];
            }

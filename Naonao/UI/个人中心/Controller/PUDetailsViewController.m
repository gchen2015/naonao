...
@interface PUDetailsViewController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation PUDetailsViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    [self setNavBarTitle:@"取货详情"];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if (section == 0) {
        return _mInfo.skuArray.count+1;
    }
    return 3;
}
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    {
        if (indexPath.row == _mInfo.skuArray.count) {
            return 58;
        }
        return 96.0;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        if (indexPath.row < _mInfo.skuArray.count){
            ...
            SKUOrderModel *sModel = _mInfo.skuArray[indexPath.row];
            ...
            CGFloat total = 0.0;
            for(SKUOrderModel *sModel in _mInfo.skuArray){
                total += [sModel.price floatValue];
            }
            
            NSString *st = [NSString stringWithFormat:@"￥%.2f", total];

...
- (CGPoint)centerPointForPosition:(METoastLocation)position withToast:(UIView *)toast;
...
- (void)makeToast:(NSString *)message {
    [self makeToast:message duration:CSToastDefaultDuration position:METoastLocationMiddle];
}

- (void)makeToast:(NSString *)message duration:(CGFloat)interval position:(METoastLocation)position {
    UIView *toast = [self viewForMessage:message title:nil image:nil];
    [self showToast:toast duration:interval position:position];
}

- (void)makeToast:(NSString *)message duration:(CGFloat)interval position:(METoastLocation)position title:(NSString *)title {
    UIView *toast = [self viewForMessage:message title:title image:nil];
    [self showToast:toast duration:interval position:position];  
}

- (void)makeToast:(NSString *)message duration:(CGFloat)interval position:(METoastLocation)position image:(UIImage *)image {
    UIView *toast = [self viewForMessage:message title:nil image:image];
    [self showToast:toast duration:interval position:position];  
}

- (void)makeToast:(NSString *)message duration:(CGFloat)interval  position:(METoastLocation)position title:(NSString *)title image:(UIImage *)image {
    UIView *toast = [self viewForMessage:message title:title image:image];
    [self showToast:toast duration:interval position:position];  
}

- (void)showToast:(UIView *)toast {
    [self showToast:toast duration:CSToastDefaultDuration position:METoastLocationBottom];
}


- (void)showToast:(UIView *)toast duration:(CGFloat)interval position:(METoastLocation)point
{
...
- (void)makeToastActivity {
    [self makeToastActivity:METoastLocationMiddle];
}


- (void)makeToastActivity:(METoastLocation)position {
    // sanity
...
- (CGPoint)centerPointForPosition:(METoastLocation)point withToast:(UIView *)toast {
    
    CGPoint mPoint = CGPointZero;
    switch (point) {
        case METoastLocationTop:
            mPoint = CGPointMake(self.bounds.size.width/2, (toast.frame.size.height / 2) + CSToastVerticalPadding);
            break;
            
        case METoastLocationMiddle:
            mPoint = CGPointMake(self.bounds.size.width / 2, self.bounds.size.height / 2);
            break;
        
        case METoastLocationBottom:
            mPoint = CGPointMake(self.bounds.size.width/2, (self.bounds.size.height - (toast.frame.size.height / 2)) - CSToastVerticalPadding);
            break;
